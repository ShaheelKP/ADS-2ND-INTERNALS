...
Event create_evt(uint32_t time,uint32_t aircraftno,uint32_t takeoff){
Event evt={time,aircraftno,takeoff};
return evt;
}
...
uint32_t time = data1->time;
uint32_t aircraftno = data1->aircraftno;
uint32_t takeoff = data1->takeoff;

data1->time = data2->time;
data1->aircraftno = data2->aircraftno;
data1->takeoff = data2->takeoff;

data2->time = time;
data2->aircraftno = aircraftno;
data2->takeoff = takeoff;
